...
function calculateDailySummary(weatherData) {
    ...
    if (existingSummary) {
        ...
        existingSummary.avgTemp = Math.round((existingSummary.avgTemp + summary.avgTemp) / 2,2);
        existingSummary.maxTemp = Math.max(existingSummary.maxTemp, summary.maxTemp);
        existingSummary.minTemp = Math.min(existingSummary.minTemp, summary.minTemp);
    } else {
...
function triggerAlert(weatherData) {
    console.log(`ALERT: Temperature exceeded ${thresholds.tempThreshold}°C in ${weatherData.name}`);
...
app.post("/weather", async (req, res) => {
    ...
    try {
        ...
        res.render("index.ejs", { weather, city, dailyLabels, tempData, thresholds, weatherSummaries, forecastData  });

    } catch (error) {
        // Log and handle errors in fetching weather data
        console.error("Error fetching weather data:", error.message);
        res.render("index.ejs", { weather: null, city: null, dailyLabels: [], tempData: [], thresholds, weatherSummaries, forecastData: []  });
    }
});
